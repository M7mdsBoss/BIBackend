...
const LOGO_URL =
  "https://res.cloudinary.com/dcsjywfui/image/upload/v1776761840/azure_z5adw6.png";
...
export async function generateVisitPDF(visit: {
  ...
  const logoColumn: any = logoDataUrl
    ? { image: logoDataUrl, height: 20, margin: [0, 5, 0, 0] }
    : { text: "", height: 20, margin: [0, 5, 0, 0] };
      ...
      // ── Header ──────────────────────────────────────────────────────────────
      {
        columns: [
          logoColumn,
          {
            ...
            stack: [
              {
                text: ["Visit Pass - ", ar("تصريح زائر")],
                fontSize: 20,
                bold: true,
                color: "#1F2937",
              },
              {
                text: [`Issued on ${issuedDate} `, ar("تاريخ الإصدار")],
                fontSize: 12,
                color: "#1F2937",
                margin: [0, 4, 0, 0],
              },
        ...
        columns: [
          { image: qrDataUrl, width: 150, height: 150 },
          {
            stack: [
              ...
              {
                text: ar(
                  "يرتبط رمز QR بصفحة التحقق الرسمية الخاصة بسجل هذه الزيارة.",
                ),
                fontSize: 10,
                color: "#9CA3AF",
                alignment: "center",
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.end();
    stream.on("finish", () =>
      resolve({ filePath, fileName, qrDataUrl, verifyUrl }),
    );
